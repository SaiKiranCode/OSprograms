...
bool SafetyAlgorithm(vector<vector<int>> AllocatedResources, vector<vector<int>> MaxResource,
 vector<vector<int>> NeedResources, vector<int> AvailableResources, vector<bool> &Finished, 
 vector<int> &SafeSequence, const int noOfProcesses, const int noOfResources)
{
...
void RemoveAndCheckDeadlock(vector<vector<int>> AllocatedResources, vector<vector<int>> MaxResource,
 vector<int> AvailableResources, vector<bool> &Finished, vector<int> &SafeSequence, 
 const int noOfProcesses, const int noOfResources, vector<int> InDeadlock)
{
    ...
    for (int i = 0; i < InDeadlock.size(); i++)
    {
        ...
        bool notInDeadlock = SafetyAlgorithm(AllocatedResources, MaxResource, NeedResources, 
            AvailableResources, Finished, SafeSequence, noOfProcesses, noOfResources);
...
int main()
{
    ...
    bool isSafe = SafetyAlgorithm(AllocatedResources, MaxResource, NeedResources, 
        AvailableResources, Finished, SafeSequence, noOfProcesses, noOfResources);
    ...
    RemoveAndCheckDeadlock(AllocatedResources, MaxResource, AvailableResources, 
        Finished, SafeSequence, noOfProcesses, noOfResources, InDeadlock);
    return 0;
}
